...
export default function fetchCanteenData(canteenXMLData){
  ...
  for(var i = 0; i < plansByDay.length; i++) {
    ...
    for (var k = 0; k < menus.length; k++) {
      ...
      var addition = menuElement.getElementsByTagName('allergikerhinweise').item(0).childNodes.item(0) !== null ? menuElement.getElementsByTagName('allergikerhinweise').item(0).childNodes.item(0).nodeValue : '';
      if(addition){
      ...
      var addition2 = menuElement.getElementsByTagName('kennzeichnungen').item(0).childNodes.item(0) !== null ? menuElement.getElementsByTagName('kennzeichnungen').item(0).childNodes.item(0).nodeValue : '';
      if(addition2){
      ...
      var vegetarian = (vegetarianAttribute != null) && ((vegetarianAttribute.nodeValue === 'vegetarisch') || (vegetarianAttribute.nodeValue === 'vegan'));
      var prices = [];
      var pushPrices = function (customerType, variableName) {
        var priceElement = menuElement.getElementsByTagName(customerType).item(0).childNodes;
        prices.push({
      ...
      var nameElement = menuElement.getElementsByTagName('name').item(0).childNodes.item(0);
      if(nameElement){
