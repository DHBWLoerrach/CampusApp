...
export default class ServiceScreen extends Component {
    ...
    return [
      {
        ...
        onPress: this._onContentSelect.bind(this, {
          name: 'cafeteriaKKH',
          label: 'Cafeteria im KKH',
  ...
  _onBackPress() {
    if (this.state.selectedContent.name !== 'submenu') {
      ...
      if (this.state.selectedContent.prevState) {
        this.setState({
          selectedContent: this.state.selectedContent.prevState
        });
      } else {
        this.setState({
          selectedContent: { name: 'submenu', label: 'Service' }
  ...
  _getContent() {
    switch (this.state.selectedContent.name) {
      case 'submenu':
        return <Submenu menuItems={this._getSubmenuItems()} />;
      case 'accounts':
        return <LinksList title="Service-Zugänge" links={linksAccounts} />;
      case 'emergency':
        return <LinksList title="Hilfe im Notfall" links={linksEmergency} />;
      case 'study':
        return <LinksList title="Studium" links={linksStudy} />;
      case 'kbc':
        return (
          <LinksList
            title="Angebote beim Bürogebäude KBC"
            links={this._sectionsForKBC()}
          />
        );
      case 'freetime':
        return <LinksList title="Freizeit" links={linksFreetime} />;
      case 'feedback':
        return <Feedback />;
      case 'settings':
        return <Settings />;
