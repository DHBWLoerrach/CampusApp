...
- [ ] Wire React Query `onlineManager` globally (current per-component approach works well).
- [ ] Distinguish "offline" vs "invalid course" in course validation.
...
- `RideMatchSheetContent` shows only banner (no empty state) since it's a bottom sheet, not a full screen.
...
- [x] Flight mode on, no cached items -> offline empty state.
- [x] Flight mode on, cached items -> banner + items remain visible.
- [x] Flight mode off -> auto-refresh once reconnected.
- [x] Pull-to-refresh while online.
