...
function isRoomToken(text: string) {
  ...
  return t.length <= 8 && /\d/.test(t) && /^[A-Za-zÄÖÜäöüß0-9\s.-]+$/.test(t);
}
...
function parseLocationForRoomAndExtra(locationTextNoUrls: string) {
  ...
  if (prefixMatch) {
    ...
    const locationExtra = extraParts.length > 0 ? extraParts.join(" · ") : null;
    return { roomText, locationExtra } as const;
  }
...
export function getScheduleCardLocationDisplay(args: {
  ...
  const { urls: descriptionUrls, textWithoutUrls: descriptionTextNoUrls } =
